...
$$\text{单局得分} = \text{对敌伤害} \times 1.0 + \text{自身回血} \times 0.6 - \text{自身受伤} \times 1.0$$

$$\text{期望得分} = \frac{\sum_{i=1}^{4} \text{单局得分}_i}{4} \quad (\text{对剑/盾/弓/药各算一次取平均})$$

权重设计：攻防等权（均 1.0），治疗降权（0.6）——因为回血不能直接赢回合；假想敌攻击力取 10（手牌 5~15 的中位数）。两个实例：

> **例 1：AI 持剑（攻击 12）**——对手出剑→互砍 12:12 得 0 分；出盾→挡住 0:0 得 0 分；出弓→剑克弓 12:0 得 12 分；出药→打断 10:0 得 10 分。期望 = (0+0+12+10)/4 = **5.5**
>
> **例 2：AI 持药（回复 13）**——对手出剑→被打断，回 13 挨 10，得 13×0.6−10 = −2.2；出盾→回血成功，得 13×0.6 = 7.8；出弓→被打断，得 −2.2；出药→双方回血，得 7.8。期望 = (−2.2+7.8−2.2+7.8)/4 = **2.8**
>
> 5.5 > 2.8 → AI 优先出剑。药虽有回血潜力，但 50% 概率被打断拉低均分。AI 对所有手牌做完这个比较，取最高分那张。
